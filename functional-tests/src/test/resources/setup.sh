...
oc process caching-service -p NAMESPACE=$(oc project -q) -p IMAGE=${IMAGE_NAME} | oc create -f -
oc process shared-memory-service -p NAMESPACE=$(oc project -q) -p IMAGE=${IMAGE_NAME} | oc create -f -
